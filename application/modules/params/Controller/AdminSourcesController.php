...
class Params_AdminSourcesController extends Core_Controller_Action
{	
    ...
    public function editAction()
    {
    	...
    	$model = Core::getMapper('params/sources')->find($id);
    ...
    public function saveAction()
    {
    	if ($data = $this->getRequest()->getPost()) {
    		try {
	    		$form = Core::getBlock('params/admin-sources/edit');
	    		if (!$form->isValid($data)) {
	    			Core::getSession('admin')->formHasErrors = true;
	    			throw new Exception($this->__("Invalid form"));
	    		}
	    			
	    		$model = Core::getMapper('params/sources')->create($form->getValues());
	    		$model->save();	    		
