...
class Frontpage_AdminSliderController extends Core_Controller_Action
{	
    ...
    public function editAction()
    {
    	...
    	$model = Core::getMapper('frontpage/slider')->find($id);
    ...
    public function saveAction()
    {
    	if (($data = $this->getRequest()->getPost()) && !$this->getRequest()->getParam('cancel')) {
    		try {
    			$form = Core::getBlock('frontpage/admin-slider/edit');
    			if (!$form->isValid($data)) {
    				Core::getSession('admin')->formHasErrors = true;
    				throw new Exception($this->__("Invalid form"));
    			}
    			
    			$model = Core::getMapper('frontpage/slider')->create($form->getValues());
    			$model->save();
